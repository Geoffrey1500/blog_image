...
if __name__ == '__main__':
    ...
    key = ['x_A', 'y_A', 'theta_A', 'x_B', 'y_B', 'theta_B', 'Overlapping', 'Inter_angle', 'Length_ratio', 'H_error', 'D_error']
    data = {}
    x_A, y_A, theta_A, x_B, y_B, theta_B, Over, inter_angle, length_ratio, H_e, D_e = [], [], [], [], [], [], [], [], [], [], []
    all_in_all = [x_A, y_A, theta_A, x_B, y_B, theta_B, Over, inter_angle, length_ratio, H_e, D_e]

    for x_cor_A in range(-40, 0, 2):
        for y_cor_A in range(2, 42, 2):
            center_A = (x_cor_A*1000, y_cor_A*1000)
            for x_cor_B in range(2, 42, 2):
                for y_cor_B in range(2, 42, 2):
                    center_B = (x_cor_B*1000, y_cor_B*1000)
                    for the_A in range(0, 60, 10):
                        rotation_A = the_A
                        for the_B in range(-60, 0, 10):
                            rotation_B = the_B
                            ...
                            if overlap_region:
                                if overlap_region[0] < 0 < overlap_region[2]:
                                    ...
                                    if overlap_ratio >= 0.5:
                                        ...
                                        mould_A = np.sqrt(x_cor_A**2 + y_cor_A**2)
                                        mould_B = np.sqrt(x_cor_B**2 + y_cor_B**2)
                                        int_angle = np.arccos((x_cor_A*x_cor_B + y_cor_A*y_cor_B)/(mould_A*mould_B))
                                        length_ = mould_A/mould_B

                                        x_A.append(x_cor_A * 1000)
                                        y_A.append(y_cor_A * 1000)
                                        theta_A.append(the_A)
                                        x_B.append(x_cor_B * 1000)
                                        y_B.append(y_cor_B * 1000)
                                        theta_B.append(the_B)
                                        Over.append(overlap_ratio)
                                        inter_angle.append(int_angle*180/np.pi)
                                        length_ratio.append(length_)
                                        H_e.append(error_in_x)
                                        D_e.append(error_in_y)
    # print(len(x_A), len(y_A), len(theta_A), len(x_B), len(y_B), len(theta_B), len(Over), len(H_e), len(D_e))
    for i in range(11):
    ...
    data_frame.to_excel('Final_result.xlsx', index=False)
